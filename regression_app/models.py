...
def _weights(x, mode, origin_include=False):
    ...
    # For an explicitly included synthetic origin, 1/x is undefined at x=0.
    # We give the synthetic origin unit weight and apply the requested weighting
    # to non-zero calibrators. This behavior is displayed transparently in the UI.
    w = np.empty_like(x)
    zero = x == 0
    nonzero = ~zero
...
def _prepare_origin(x, y, origin_mode):
    x = np.asarray(x, float)
    y = np.asarray(y, float)
    # Experimental zero rows are kept unless they make weighting invalid.
...
def _polyfit_force_origin(x, y, degree, w):
    # Weighted least squares with intercept fixed at zero.
    # degree 1: y = b*x
...
def fit_polynomial(x, y, degree=1, weight_mode="none", name=None,
    ...
    notes = [
        ...
        "Pearson r² = corr(x,y)² and does not use the fitted residuals.",
        "Fit R² = 1 − Σ(y−ŷ)² / Σ(y−ȳ)².",
    ...
    return FitResult(
        name=name or ("Linear" if degree == 1 else "Quadratic"),
        params=coeff, predict=predict, invert=invert, yhat=yhat, residuals=y-yhat,
        backcalc_x=backcalc, bias_pct=bias, weights=w_original, stats=stats,
        pass_mask=pass_mask, contiguous_range=crange, contiguous_count=ccount,
        equation=equation, parameter_names=parameter_names,
        calculation_notes="\n".join(notes),
...
def fit_pade(x, y, order="1/1", weight_mode="none", bias_limit=15.0,
    ...
    w_fit=_weights(x_fit,weight_mode,origin_include=synthetic_origin)
    sqrtw=np.sqrt(w_fit)
    force=origin_mode==ORIGIN_FORCE
    ...
    def resid(q):
        ...
        if np.any(np.abs(den)<1e-8):
            return np.full_like(y_fit,1e12)
    ...
    if order=="1/1":
        equation="y = (a + b·x) / (1 + c·x)"
        parameter_names=("a","b","c")
    else:
        equation="y = (a + b·x + c·x²) / (1 + d·x)"
        parameter_names=("a","b","c","d")

    calc_notes = (
        f"Nonlinear weighted least squares minimizes Σ wᵢ(yᵢ − ŷᵢ)². "
        f"Origin handling: {origin_mode}. "
        "The denominator constant is fixed at 1 for identifiability.\n"
        "Pearson r² = corr(x,y)². Fit R² uses ordinary residual sums of squares. "
        "Weighted R² uses weighted residual and weighted total sums of squares."
    )

    return FitResult(
        name=f"Padé [{order}]", params=p, predict=predict, invert=invert, yhat=yhat,
        residuals=y-yhat, backcalc_x=backcalc, bias_pct=bias, weights=w_original,
        stats=stats, pass_mask=pass_mask, contiguous_range=crange, contiguous_count=ccount,
        equation=equation, parameter_names=parameter_names,
