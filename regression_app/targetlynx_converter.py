...
TEXT_FIELDS = {
    "Compound", "Name", "ID", "Sample Text", "Type",
    "Acq.Time", "Acq.Date", "Vial", "Primary Flags",
}
...
def _typed_targetlynx_value(column, value):
    """Convert unambiguous numeric TargetLynx cells to numeric Python values."""
    if value is None:
        return ""
...
def coerce_targetlynx_types(df):
    """Return a copy with numeric-looking result cells stored as numbers."""
    out = df.copy()
    for column in out.columns:
        if column == "Is Internal Standard":
            continue
        out[column] = out[column].map(
            lambda value, c=column: _typed_targetlynx_value(c, value)
        )
    return out
...
def parse_targetlynx_compound_summary(path):
    ...
    with path.open("r", encoding="utf-8-sig", errors="replace", newline="") as f:
        ...
        for raw in reader:
            ...
            if not report_title and first:
                report_title = first
            if first.lower().startswith("printed "):
                printed_line = first
            ...
            if compound and len(raw) > 2:
                second = str(raw[1]).strip()
                third = str(raw[2]).strip()
    ...
    if not records:
        raise ValueError(
            "No TargetLynx compound blocks were detected. This converter expects "
            "a Waters 'Quantify Compound Summary Report' containing lines such as "
            "'Compound 1: Amoxicillin'."
        )

    df = pd.DataFrame(records)
    df = coerce_targetlynx_types(df)
...
def available_value_fields(df):
    ...
    excluded = {
        "Compound", "Is Internal Standard", "#", "Name", "ID",
        "Sample Text", "Type", "Acq.Time", "Acq.Date", "Vial"
    }
    for c in df.columns:
        if c not in excluded and c not in fields:
            fields.append(c)
    return fields
...
def filter_results(df, compounds=None, sample_type="All"):
    out = df.copy()
    if compounds:
        out = out[out["Compound"].astype(str).isin([str(x) for x in compounds])]
...
def build_long_output(df, compounds, value_fields, sample_type="All", include_metadata=True):
    if not compounds:
        raise ValueError("Select at least one compound.")
    if not value_fields:
        raise ValueError("Select at least one TargetLynx result field.")
    out = filter_results(df, compounds, sample_type)

    id_cols = ["Compound"]
    if include_metadata:
        id_cols += [c for c in DEFAULT_METADATA if c in out.columns]
    else:
        id_cols += [c for c in ("Sample Text", "Type") if c in out.columns]
...
def build_wide_output(df, compounds, value_fields, sample_type="All", include_metadata=True):
    ...
    if include_metadata:
        meta_cols = [c for c in DEFAULT_METADATA if c in long.columns and c != sample_key]
    else:
        meta_cols = [c for c in ("Sample Text", "Type") if c in long.columns and c != sample_key]

    meta = (
        long[[sample_key] + meta_cols]
        .drop_duplicates(subset=[sample_key], keep="first")
...
def export_1d_workbook(df, compounds, value_fields, path, sample_type="All", include_metadata=True):
    with pd.ExcelWriter(path, engine="openpyxl") as writer:
        ...
        for i, compound in enumerate(compounds):
            one = build_long_output(
                df, [compound], value_fields,
